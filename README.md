...
本專案聚焦於 Hexagon-Based Search (HEXDS) 運動估計演算法，提供「Python 端演算法分析 → 黃金向量生成 → Verilog RTL 驗證 → 合成 (DC)」的一條龍流程，方便在 VLSI/FPGA 專案中快速驗證與交付硬體 IP。

## 功能亮點

- **多演算法 Benchmark：** `Motion_EstimationG.py` 同時實作 Full Search、Three-Step Search、Diamond Search 與 HEXDS，支援多執行緒＋Numba JIT 加速。
...
- **HEXDS 硬體 IP：** `HEXDS/me_hexds_topB.v` 為完整 RTL，含 testbench (`HEXDS/golden.v`)、架構文件與 IP 規格，支援自動化 Makefile 模擬流程。
...
│   ├── HEXDS_Architecture.md  # 架構圖 (Mermaid)
...
- `ME_Ultimate_<timestamp>/*_hw_mapping_trace.json`：HEXDS FSM/搜尋紀錄，可對照 RTL 狀態機。

### 常用參數

- `HEXDS_SEARCH_RANGE`：覆寫預設 ±32 搜尋範圍（Python 與 RTL 要一致）。
- `video/` 內可換成自備的 Y4M 影片；若放在其他路徑，請直接修改 `VIDEОS` 或使用軟連結。

## 黃金向量 (Golden Patterns) 產生

以 Python 端的 HEXDS 演算法輸出硬體驗證資料：
...
- `HEXDS_MAX_FRAMES`：例如 `export HEXDS_MAX_FRAMES=3` 只產生前 3 幀，加速驗證。
- `HEXDS_SEARCH_RANGE`：需與 RTL/主程式一致，預設 32。
- `VIDEO_PATH`（直接編輯腳本）或利用軟連結指向其他影片。

## HEXDS Verilog 模組與模擬

`HEXDS/me_hexds_topB.v` 為 RTL，`HEXDS/golden.v` 為 system-level testbench，會自動載入 `golden_patterns/` 的資料並跑完整 Frame/MB sweep，內建邊界條件測試與 `waveform_boundary.vcd` dump。
...
- `HEXDS_Architecture.md`：以 Mermaid 描述 Control FSM、Address/SAD/Decision datapath。
...
1. 編輯 `syn/filelist.f` 與 `syn/constraints/hexds_constraints.sdc`（時脈預設 10ns）。  
2. 設定 `TARGET_LIBS` / `LINK_LIBS` 環境變數。  
3. 執行：`dc_shell -f syn/scripts/dc_compile_hexds.tcl | tee syn/logs/dc.log`.  
4. 產出 `syn/hexds_top_syn.v`、`syn/hexds_top_syn.sdf`、`syn/reports/*.rpt`。  
5. 進行 Gate-level Simulation 時，可沿用 `HEXDS/golden.v` 測試平台。

更多 ASIC Flow 筆記請參考 `Cell-Based_Design_Flow.md` 與隨附 PDF。

## Vivado FPGA 原型 (Nexys A7)

`FinalA/` 目錄是針對 Digilent Nexys A7 (XC7A35T-1CPG236C) 建立的 Vivado 2025.1 專案，主體由 `Smart_Top` 結合 `hexds_top`、雙埠 BRAM 影像緩衝與自訂 UART 協定 (921600 bps)。LED[15:8] 會顯示最近一次運動向量的 X 分量，LED[7:0] 顯示 Y 分量，方便肉眼確認。

### 建構 Bitstream

1. 至 `FinalA/` 打開 `FinalA.xpr`（Vivado 2025.1+）。  
2. 確認 `sources_1/new` 中的 `Smart_Top.v`、`UART_Modules.v`、`hexds_top.v` 都在工程內，目標還原點為 `xc7a35tcpg236-1`。  
...
- 會自動以 `CMD_LOAD_REF (R)` / `CMD_LOAD_CUR (F)` 傳輸整幀，再用 `CMD_RUN_CASE (C)` 觸發 HEXDS，並對照回傳的 MV/SAD。  
...
這段流程解答「如何在 Basys 3/Nexys A7 上反覆測 HEXDS、並讓程式斷電後保留？」的共通疑問。

#### 快速摘要

1. **重置方式：** 推薦自行設計「軟重置」按鈕控制 `rst`，必要時才用板上紅色 `PROG`。
2. **斷電保留：** 只有把 bitstream 轉 `.bin` 並燒進 QSPI Flash 才會記住程式。
3. **JP1 跳線：** 若要上電自啟動 HEXDS，務必把 JP1 從預設 JTAG 改到 **QSPI** 位置。
...
> **建議：** 寫完 Flash 立即調整 JP1→QSPI，斷電重上電驗證 HEXDS 是否自動啟動；平常測試再利用軟重置即可。
...
3. 再次上電或接行動電源，HEXDS 會自動啟動；需要重新燒錄才按 `PROG` 或用 Vivado。  
